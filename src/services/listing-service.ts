...
import { ListingData } from "../types/reservoir-types/listing-data.types";
import { submitListingData } from "./api/marketplace-reservoir-api";
import { executeTransactions } from "./seaport";

export const handleListingData = async (chainId: number, data: ListingData) => {
  ...
  executeTransactions(requiredApprovals, signer).then(async () => {
    ...
    submitListingData(chainId, orderPost).then((result) =>
